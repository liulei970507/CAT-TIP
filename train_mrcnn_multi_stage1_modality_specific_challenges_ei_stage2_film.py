...
from tracker_multi_stage1_modality_specific_challenges_ei_stage2_film import *
import numpy as np
...
def train_mdnet():
    ...
    for i in range(pretrain_opts['n_cycles']):
        ...
        prec = np.zeros(K)
        total_cls_loss = np.zeros(K)
        totalInterClassLoss = np.zeros(K)
        total_loss_filter = np.zeros(K)
        total_loss_response = np.zeros(K)
        for j, k in enumerate(k_list):
            ...
            total_cls_loss[k] = cls_loss.data[0]
            ...
            # print "Cycle %2d, K %2d (%2d), loss_filter %.3f, loss_response %.3f, BinLoss %.3f, Prec %.3f, interLoss %.3f, Time %.3f" % \
            #           (i, j, k, loss_filter.data[0], loss_response.data[0], cls_loss.data[0], prec[k], totalInterClassLoss[k], toc)
        ...
        if cur_score > best_score:
            ...
            # torch.save(states, pretrain_opts['model_path']+'GTOT_multi'+'.pth')
            # if cur_score>0.9:
...
if __name__ == "__main__":
    ...
    parser.add_argument("-model_path", default ="./models/GTOT_multi_ill_stage2_film/", help = "model path")
    ...
    parser.add_argument("-init_model_path", default="/home/user/liulei/cat/models/GTOT_multi_ill_stage1/GTOT_multi2128.pth")
