...
class MDNet(nn.Module):
        ...
        for (name_v, module_v),(name_i, module_i) in zip(self.layers_v.named_children(),self.layers_i.named_children()):
            ...
            if run:
                if name_v in ['conv1','conv2','conv3']:
                    if name_v == 'conv1':
                        x1_paralle = self.parallel1_v(x1)
                        x2_paralle = self.parallel1_i(x2)
                    elif name_v == 'conv2':
                        x1_paralle = self.parallel2_v(x1)
                        x2_paralle = self.parallel2_i(x2)
                    else:
                        x1_paralle = self.parallel3_v(x1)
                        x2_paralle = self.parallel3_i(x2)
                    x1 = module_v(x1) + x1_paralle
                    x2 = module_i(x2) + x2_paralle
    ...
    def load_mat_model(self, model_path):
