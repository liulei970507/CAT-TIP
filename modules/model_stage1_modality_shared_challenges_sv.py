        ...
        for (name_v, module_v),(name_i, module_i) in zip(self.layers_v.named_children(),self.layers_i.named_children()):
            ...
            if run:
                if name_v in ['conv1','conv2','conv3']:
                    if name_v == 'conv1':
                        x1_parallel = self.parallel1(x1)
                        x1_parallel = self.parallel1_f(torch.cat([self.parallel1_b1(x1_parallel), self.parallel1_b2(x1_parallel), self.parallel1_b3(x1_parallel)],1))
                        x2_parallel = self.parallel1(x2)
                        x2_parallel = self.parallel1_f(torch.cat([self.parallel1_b1(x2_parallel), self.parallel1_b2(x2_parallel), self.parallel1_b3(x2_parallel)],1))
                    elif name_v == 'conv2':
                        x1_parallel = self.parallel2(x1)
                        x1_parallel = self.parallel2_f(torch.cat([self.parallel2_b1(x1_parallel), self.parallel2_b2(x1_parallel), self.parallel2_b3(x1_parallel)],1))
                        x2_parallel = self.parallel2(x2)
                        x2_parallel = self.parallel2_f(torch.cat([self.parallel2_b1(x2_parallel), self.parallel2_b2(x2_parallel), self.parallel2_b3(x2_parallel)],1))
                    else:
                        x1_parallel = self.parallel3(x1)
                        x1_parallel = self.parallel3_f(torch.cat([self.parallel3_b1(x1_parallel), self.parallel3_b2(x1_parallel), self.parallel3_b3(x1_parallel)],1))
                        x2_parallel = self.parallel3(x2)
                        x2_parallel = self.parallel3_f(torch.cat([self.parallel3_b1(x2_parallel), self.parallel3_b2(x2_parallel), self.parallel3_b3(x2_parallel)],1))
