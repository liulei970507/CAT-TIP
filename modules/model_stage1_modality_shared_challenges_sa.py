            ...
            if run:
                if name_v in ['conv1','conv2','conv3']:
                    if name_v == 'conv1':
                        ...
                        x1_parallel = self.parallel1_sa(x1_parallel)*x1_parallel
                        x2_parallel = self.parallel1_sa(x2_parallel)*x2_parallel
                    elif name_v == 'conv2':
                        self.x1_parallel2 = x1_parallel = self.parallel2(x1)
                        self.x2_parallel2 = x2_parallel = self.parallel2(x2)
                        x1_parallel = self.parallel2_sa(x1_parallel)*x1_parallel
                        x2_parallel = self.parallel2_sa(x2_parallel)*x2_parallel
                    else:
                        self.x1_parallel3 = x1_parallel = self.parallel3(x1)
                        self.x2_parallel3 = x2_parallel = self.parallel3(x2)
                        x1_parallel = self.parallel3_sa(x1_parallel)*x1_parallel
                        x2_parallel = self.parallel3_sa(x2_parallel)*x2_parallel
