        ...
        for (name_v, module_v),(name_i, module_i) in zip(self.layers_v.named_children(),self.layers_i.named_children()):
            ...
            if run:
                if name_v in ['conv1','conv2','conv3']:
                    if name_v == 'conv1':
                        x2_parallel = self.parallel1_i(x1)
                        gama_out = self.guidance1_gama_conv0_21(x2_parallel)
                        gama_gate = self.sigmoid(self.guidance1_gama_conv1_21(gama_out))
                        gama = gama_out*gama_gate
                        
                        beta_out = self.guidance1_beta_conv0_21(x2_parallel)
                        beta_gate = self.sigmoid(self.guidance1_beta_conv1_21(beta_out))
                        beta = beta_out*beta_gate

                        x1_parallel = self.parallel1_v(x1)*gama + beta

                    elif name_v == 'conv2':
                        x2_parallel = self.parallel2_i(x2)
                        gama_out = self.guidance2_gama_conv0_21(x2_parallel)
                        gama_gate = self.sigmoid(self.guidance2_gama_conv1_21(gama_out))
                        gama = gama_out*gama_gate
                        
                        beta_out = self.guidance2_beta_conv0_21(x2_parallel)
                        beta_gate = self.sigmoid(self.guidance2_beta_conv1_21(beta_out))
                        beta = beta_out*beta_gate

                        x1_parallel = self.parallel2_v(x1)*gama + beta
                    else:
                        x2_parallel = self.parallel3_i(x2)
                        gama_out = self.guidance3_gama_conv0_21(x2_parallel)
                        gama_gate = self.sigmoid(self.guidance3_gama_conv1_21(gama_out))
                        gama = gama_out*gama_gate
                        
                        beta_out = self.guidance3_beta_conv0_21(x2_parallel)
                        beta_gate = self.sigmoid(self.guidance3_beta_conv1_21(beta_out))
                        beta = beta_out*beta_gate

                        x1_parallel = self.parallel3_v(x1)*gama + beta
