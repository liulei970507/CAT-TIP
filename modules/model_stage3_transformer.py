...
class MDNet(nn.Module):
    def __init__(self, model_path=None,K=1):
        ...
        self.transformer3_decoder_rgbt=nn.Sequential(OrderedDict([
                ('transformer3_decoder_WK',nn.Sequential(nn.Linear(64,64))),
                ('transformer3_decoder_WV',nn.Sequential(nn.Linear(64,64))),  
                ('transformer3_decoder_fc_reduce',nn.Sequential(nn.Conv2d(512,64,1,1,bias=False))),
                ('transformer3_decoder_fc_rise',nn.Sequential(nn.Conv2d(64,512,1)))
                ]))
        self.transformer3_decoder_trgb=nn.Sequential(OrderedDict([
                ('transformer3_decoder_WK',nn.Sequential(nn.Linear(64,64))),
                ('transformer3_decoder_WV',nn.Sequential(nn.Linear(64,64))),  
                ('transformer3_decoder_fc_reduce',nn.Sequential(nn.Conv2d(512,64,1,1,bias=False))),
                ('transformer3_decoder_fc_rise',nn.Sequential(nn.Conv2d(64,512,1)))
                ]))
        for m in self.transformer3_decoder_rgbt.modules():
        ...
        for m in self.transformer3_decoder_trgb.modules():
            if isinstance(m, nn.Conv2d):
                nn.init.normal_(m.weight, 0, 0.01)
            if isinstance(m, nn.Linear):
                m.weight.data.normal_(0, math.sqrt(2. / m.out_features))
    ...
    def build_param_dict(self):
        ...
        for name, module in self.transformer3_decoder_rgbt.named_children():
            append_params(self.params, module, 'transformer3_decoder_rgbt'+name)
        
        for name, module in self.transformer3_decoder_trgb.named_children():
            append_params(self.params, module, 'transformer3_decoder_trgb'+name)
    ...
    def CrossAttention_layer3_trgb(self,x,V):
        x_1=self.transformer3_decoder_trgb[2](x)   
        batch,dim,w,h = x_1.shape    
        V_1=self.transformer3_decoder_trgb[2](V) 
        x_1=x_1.permute(0,2,3,1)
        V_1=V_1.permute(0,2,3,1)
        V_k=V_1.reshape(batch,w*h,dim)  
        V_v=V_1.reshape(batch,w*h,dim)  
        x_q=x_1.reshape(batch,w*h,dim)

        w_k=self.transformer3_decoder_trgb[0](V_k)   
        w_k = F.normalize(w_k, p=2, dim=-1) 
        w_k=w_k.permute(0,1,2)    

        w_q = self.transformer3_decoder_trgb[0](x_q)  
        ...
        w_v=self.transformer3_decoder_trgb[1](V_v)
        ...
        output=self.transformer3_decoder_trgb[3](output)    
        x=x+output
        return x

    def CrossAttention_layer3_rgbt(self,x,V):
        x_1=self.transformer3_decoder_rgbt[2](x)   
        batch,dim,w,h = x_1.shape    
        V_1=self.transformer3_decoder_rgbt[2](V) 
        x_1=x_1.permute(0,2,3,1)
        V_1=V_1.permute(0,2,3,1)
        V_k=V_1.reshape(batch,w*h,dim)  
        V_v=V_1.reshape(batch,w*h,dim)  
        ...
        w_k=self.transformer3_decoder_rgbt[0](V_k)   
        w_k = F.normalize(w_k, p=2, dim=-1) 
        w_k=w_k.permute(0,1,2)    

        w_q = self.transformer3_decoder_rgbt[0](x_q)  
        w_q = F.normalize(w_q, p=2, dim=-1)
        w_q=w_q.permute(0,2,1)   
        ...
        w_v=self.transformer3_decoder_rgbt[1](V_v)
        w_v = F.normalize(w_v, p=2, dim=-1)
        w_v=w_v.permute(0,2,1)    
        ...
        output=output.permute(0,2,1)
        output=output.reshape(batch,dim,w,h)  
        output=self.transformer3_decoder_rgbt[3](output)    
        x=x+output
        return x
    ...
    def load_model(self, model_path):
        ...
        self.transformer3_decoder_rgbt.load_state_dict(states['transformer3_decoder_rgbt'])
        self.transformer3_decoder_trgb.load_state_dict(states['transformer3_decoder_trgb'])
