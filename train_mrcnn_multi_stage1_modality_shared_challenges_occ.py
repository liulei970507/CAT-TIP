...
from tracker_multi_stage1_modality_shared_challenges_occ import *
import numpy as np
...
def train_mdnet():
    ...
    orthogonal_regularization_loss = orthogonal_regularization()
    orthogonal_regularization_loss_feature = orthogonal_regularization_feature()
    evaluator = Precision()
    ...
    for i in range(pretrain_opts['n_cycles']):
        ...
        for j, k in enumerate(k_list):
            ...
            # filter
            loss_filter = orthogonal_regularization_loss(model)
            total_loss_filter[k] = loss_filter.data[0]
            loss_response = orthogonal_regularization_loss_feature(model.x1_parallel1)+orthogonal_regularization_loss_feature(model.x1_parallel2)+orthogonal_regularization_loss_feature(model.x1_parallel3)+\
                orthogonal_regularization_loss_feature(model.x2_parallel1)+orthogonal_regularization_loss_feature(model.x2_parallel2)+orthogonal_regularization_loss_feature(model.x2_parallel3)
            total_loss_response[k] = loss_response.data[0]
            (100*loss_filter+loss_response+cls_loss+0.1*interclass_loss).backward()
...
if __name__ == "__main__":
    ...
    parser.add_argument("-model_path", default ="./models/GTOT_multi_occ/", help = "model path")
