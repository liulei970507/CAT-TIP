...
from tracker_multi_stage1_modality_shared_challenges_sa import *
import numpy as np
...
def train_mdnet():
    ...
    if pretrain_opts['set_type'] == 'RGBT234':
        img_home = '/home/user/liulei/Datasets/RGBT234_SA/'  
        data_path = './data/RGBT234_SA.pkl'
    elif pretrain_opts['set_type'] == 'GTOT':
        img_home = '/home/user/liulei/Datasets/GTOT_SA/'
        data_path = './data/GTOT_SA.pkl'
    ...
    for i in range(pretrain_opts['n_cycles']):
        ...
        for j, k in enumerate(k_list):
            # try:
            tic = time.time()
            cropped_SAenes_visible, cropped_SAenes_infrared, pos_rois, neg_rois= dataset[k].next()
            for sidx in range(0, len(cropped_SAenes_visible)):
                cur_SAene_visible = cropped_SAenes_visible[sidx]
                cur_SAene_infrared = cropped_SAenes_infrared[sidx]
                cur_pos_rois = pos_rois[sidx]
                cur_neg_rois = neg_rois[sidx]

                cur_SAene_visible = Variable(cur_SAene_visible)
                cur_SAene_infrared = Variable(cur_SAene_infrared)
                cur_pos_rois = Variable(cur_pos_rois)
                cur_neg_rois = Variable(cur_neg_rois)
                if pretrain_opts['use_gpu']:
                    cur_SAene_visible = cur_SAene_visible.cuda()
                    cur_SAene_infrared = cur_SAene_infrared.cuda()
                    cur_pos_rois = cur_pos_rois.cuda()
                    cur_neg_rois = cur_neg_rois.cuda()
                cur_feat_map = model(cur_SAene_visible, cur_SAene_infrared, k, out_layer='conv3_relu')
...
if __name__ == "__main__":
    ...
    parser.add_argument("-model_path", default ="./models/GTOT_multi_sa/", help = "model path")
    ...
    parser.add_argument("-batch_frames", default = 4, type = int)
