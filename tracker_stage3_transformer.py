...
def train(model, criterion, optimizer, pos_feats, neg_feats, maxiter, in_layer='fc4'):
    ...
    for iter in range(maxiter):
        ...
        neg_score = model(batch_neg_feats, batch_pos_feats,in_layer=in_layer)
...
def run_mdnet(img_list_visible, img_list_infrared, init_bbox, gt=None, seq='seq_name ex)Basketball', savefig_dir='', display=False):
    ...
    iou_result = np.zeros((len(img_list_visible),1))
    iou_result_bb = np.zeros((len(img_list_visible),1))
    # execution time array
    exec_time_result = np.zeros((len(img_list_visible),1))
    ...
    tic = time.time()
    # Load first image
    cur_image_visible = Image.open(img_list_visible[0]).convert('RGB')
    cur_image_visible = np.asarray(cur_image_visible)

    cur_image_infrared = Image.open(img_list_infrared[0]).convert('RGB')
    cur_image_infrared = np.asarray(cur_image_infrared)

    # Draw pos/neg samples
    
    ishape = cur_image_visible.shape
    ...
    for bidx in range(0,scene_boxes.shape[0]):
        ...
        cropped_image_visible, cur_image_visible_var = img_crop_model.crop_image(cur_image_visible, np.reshape(scene_boxes[bidx],(1,4)), crop_img_size)
        cropped_image_visible = cropped_image_visible - 128.
        cropped_image_infrared, cur_image_visible_var = img_crop_model.crop_image(cur_image_infrared, np.reshape(scene_boxes[bidx],(1,4)), crop_img_size)
        cropped_image_infrared = cropped_image_infrared - 128.
        feat_map = model(cropped_image_visible, cropped_image_infrared, out_layer='conv3_relu')
        ...
        pos_feats_rgb_temp = cur_pos_feats[:, :512, :, :]
        pos_feats_t_temp = cur_pos_feats[:, 512:, :, :]
        pos_feats_rgb = model.CrossAttention_layer3_rgbt(pos_feats_rgb_temp, pos_feats_t_temp)
        pos_feats_t = model.CrossAttention_layer3_trgb(pos_feats_t_temp, pos_feats_rgb_temp)
        pos_feats = torch.cat([pos_feats_rgb,pos_feats_t],1)
        cur_pos_feats = pos_feats.view(pos_feats.size(0), -1).data.clone()
        ...
        neg_feats_rgb_temp = cur_neg_feats[:, :512, :, :]
        neg_feats_t_temp = cur_neg_feats[:, 512:, :, :]
        neg_feats_rgb = model.CrossAttention_layer3_rgbt(neg_feats_rgb_temp, neg_feats_t_temp)
        neg_feats_t = model.CrossAttention_layer3_trgb(neg_feats_t_temp, neg_feats_rgb_temp)
        neg_feats = torch.cat([neg_feats_rgb,neg_feats_t],1)
        cur_neg_feats = neg_feats.view(neg_feats.size(0), -1).data.clone()
        ...
        bbreg_feats_rgb_temp = cur_bbreg_feats[:, :512, :, :]
        bbreg_feats_t_temp = cur_bbreg_feats[:, 512:, :, :]
        bbreg_feats_rgb = model.CrossAttention_layer3_rgbt(bbreg_feats_rgb_temp, bbreg_feats_t_temp)
        bbreg_feats_t = model.CrossAttention_layer3_trgb(bbreg_feats_t_temp, bbreg_feats_rgb_temp)
        bbreg_feats = torch.cat([bbreg_feats_rgb,bbreg_feats_t],1)
        cur_bbreg_feats = bbreg_feats.view(bbreg_feats.size(0), -1).data.clone()

        feat_dim = cur_pos_feats.size(-1)

        if bidx==0:
            pos_feats = cur_pos_feats#.view(cur_pos_feats.size(0),-1)
            neg_feats = cur_neg_feats#.view(cur_neg_feats.size(0),-1)
            ##bbreg feature
            bbreg_feats = cur_bbreg_feats#.view(cur_bbreg_feats.size(0),-1)
            bbreg_examples = cur_bbreg_examples
        else:
            # import pdb
            # pdb.set_trace()
            pos_feats = torch.cat((pos_feats.view(pos_feats.size(0),-1),cur_pos_feats),dim=0)
            neg_feats = torch.cat((neg_feats.view(neg_feats.size(0),-1),cur_neg_feats),dim=0)
            ##bbreg feature
            bbreg_feats = torch.cat((bbreg_feats.view(bbreg_feats.size(0),-1), cur_bbreg_feats),dim=0)
    ...
    for iidx in range(replicateNum):
        ...
        cur_extra_cropped_image_visible, _ = img_crop_model.crop_image(cur_image_visible, np.reshape(extra_scene_box,(1,4)),extra_crop_img_size)
        cur_extra_cropped_image_visible = cur_extra_cropped_image_visible.detach()
        cur_extra_cropped_image_infrared, _ = img_crop_model.crop_image(cur_image_infrared, np.reshape(extra_scene_box,(1,4)),extra_crop_img_size)
        cur_extra_cropped_image_infrared = cur_extra_cropped_image_infrared.detach()
        ...
        if iidx==0:
            extra_cropped_image_visible = cur_extra_cropped_image_visible
            extra_cropped_image_infrared = cur_extra_cropped_image_infrared
            ...
            extra_cropped_image_visible = torch.cat((extra_cropped_image_visible,cur_extra_cropped_image_visible),dim=0)
            extra_cropped_image_infrared = torch.cat((extra_cropped_image_infrared,cur_extra_cropped_image_infrared),dim=0)
    ...
    extra_cropped_image_visible -= 128.
    extra_cropped_image_infrared -= 128.

    extra_feat_maps = model(extra_cropped_image_visible, extra_cropped_image_infrared, out_layer='conv3_relu')
    # Draw pos/neg samples
    ishape = cur_image_visible.shape
    ...
    pos_feats_rgb_temp = extra_pos_feats[:, :512, :, :]
    pos_feats_t_temp = extra_pos_feats[:, 512:, :, :]
    pos_feats_rgb = model.CrossAttention_layer3_rgbt(pos_feats_rgb_temp, pos_feats_t_temp)
    pos_feats_t = model.CrossAttention_layer3_trgb(pos_feats_t_temp, pos_feats_rgb_temp)
    pos_feats = torch.cat([pos_feats_rgb,pos_feats_t],1)
    extra_pos_feats = pos_feats.view(pos_feats.size(0), -1).data.clone()
    ...
    extra_neg_feats_rgb_temp = extra_neg_feats[:, :512, :, :]
    extra_neg_feats_t_temp = extra_neg_feats[:, 512:, :, :]
    extra_neg_feats_rgb = model.CrossAttention_layer3_rgbt(extra_neg_feats_rgb_temp, extra_neg_feats_t_temp)
    extra_neg_feats_t = model.CrossAttention_layer3_trgb(extra_neg_feats_t_temp, extra_neg_feats_rgb_temp)
    extra_neg_feats = torch.cat([extra_neg_feats_rgb,extra_neg_feats_t],1)
    extra_neg_feats = extra_neg_feats.view(extra_neg_feats.size(0), -1).data.clone()
    ...
    extra_bbreg_feats_rgb_temp = extra_bbreg_feats[:, :512, :, :]
    extra_bbreg_feats_t_temp = extra_bbreg_feats[:, 512:, :, :]
    extra_bbreg_feats_rgb = model.CrossAttention_layer3_rgbt(extra_bbreg_feats_rgb_temp, extra_bbreg_feats_t_temp)
    extra_bbreg_feats_t = model.CrossAttention_layer3_trgb(extra_bbreg_feats_t_temp, extra_bbreg_feats_rgb_temp)
    extra_bbreg_feats = torch.cat([extra_bbreg_feats_rgb,extra_bbreg_feats_t],1)
    extra_bbreg_feats = extra_bbreg_feats.view(extra_bbreg_feats.size(0), -1).data.clone()

    ## concatenate extra features to original_features
    pos_feats = torch.cat((pos_feats.view(pos_feats.size(0), -1),extra_pos_feats),dim=0)
    neg_feats = torch.cat((neg_feats.view(neg_feats.size(0), -1),extra_neg_feats), dim=0)
    ## concatenate extra bbreg feats to original_bbreg_feats
    bbreg_feats = torch.cat((bbreg_feats.view(bbreg_feats.size(0), -1), extra_bbreg_feats), dim=0)
    ...
    #     dpi = 80.0
    #     figsize = (cur_image_visible.shape[1]/dpi, cur_image_visible.shape[0]/dpi)
    ...
    #     im = ax.imshow(cur_image_visible)
    #     if gt is not None:
    ...
    for i in range(1,len(img_list_visible)):
        ...
        tic = time.time()
        # Load image
        cur_image_visible = Image.open(img_list_visible[i]).convert('RGB')
        cur_image_visible = np.asarray(cur_image_visible)
        cur_image_infrared = Image.open(img_list_infrared[i]).convert('RGB')
        cur_image_infrared = np.asarray(cur_image_infrared)
        # Estimate target bbox
        if cur_image_visible.shape == cur_image_infrared.shape:
            pass
        else:
            print seq, 'the resolution of visible and infrared image is not same size!'
        ishape = cur_image_visible.shape
        ...
        if padded_scene_box[0] > cur_image_visible.shape[1]:
            padded_scene_box[0] = cur_image_visible.shape[1]-1
        if padded_scene_box[1] > cur_image_visible.shape[0]:
            padded_scene_box[1] = cur_image_visible.shape[0]-1
        ...
        crop_img_size = (padded_scene_box[2:4] * ((opts['img_size'], opts['img_size']) / target_bbox[2:4])).astype(
            'int64')
        cropped_image_visible,cur_image_visible_var = img_crop_model.crop_image(cur_image_visible, np.reshape(padded_scene_box,(1,4)),crop_img_size)
        cropped_image_visible = cropped_image_visible - 128.
        cropped_image_infrared,cur_image_infrared_var = img_crop_model.crop_image(cur_image_infrared, np.reshape(padded_scene_box,(1,4)),crop_img_size)
        cropped_image_infrared = cropped_image_infrared - 128.
        model.eval()
        feat_map = model(cropped_image_visible, cropped_image_infrared, out_layer='conv3_relu')
        ...
        pos_feats_rgb_temp = sample_feats[:, :512, :, :]
        pos_feats_t_temp = sample_feats[:, 512:, :, :]
        pos_feats_rgb = model.CrossAttention_layer3_rgbt(pos_feats_rgb_temp, pos_feats_t_temp)
        pos_feats_t = model.CrossAttention_layer3_trgb(pos_feats_t_temp, pos_feats_rgb_temp)
        pos_feats = torch.cat([pos_feats_rgb,pos_feats_t],1)
        sample_feats = pos_feats.view(pos_feats.size(0), -1).data.clone()
        ...
        if success:
            ...
            for bidx in range(0, scene_boxes.shape[0]):
                ...
                cropped_image_visible, cur_image_visible_var = img_crop_model.crop_image(cur_image_visible,np.reshape(scene_boxes[bidx], (1, 4)),crop_img_size)
                cropped_image_visible = cropped_image_visible - 128.
                cropped_image_infrared, cur_image_infrared_var = img_crop_model.crop_image(cur_image_infrared,np.reshape(scene_boxes[bidx], (1, 4)),crop_img_size)
                cropped_image_infrared = cropped_image_infrared - 128.
                feat_map = model(cropped_image_visible, cropped_image_infrared, out_layer='conv3_relu')
                ...
                cur_neg_feats = model.roi_align_model(feat_map, cur_neg_rois)
                # cur_neg_feats = cur_neg_feats.view(cur_neg_feats.size(0), -1).data.clone()
                pos_feats_rgb_temp = cur_neg_feats[:, :512, :, :]
                pos_feats_t_temp = cur_neg_feats[:, 512:, :, :]
                pos_feats_rgb = model.CrossAttention_layer3_rgbt(pos_feats_rgb_temp, pos_feats_t_temp)
                pos_feats_t = model.CrossAttention_layer3_trgb(pos_feats_t_temp, pos_feats_rgb_temp)
                pos_feats = torch.cat([pos_feats_rgb,pos_feats_t],1)
                cur_neg_feats = pos_feats.view(pos_feats.size(0), -1).data.clone()
        ...
        if opts['visual_log']:
            if gt is None:
                print "Frame %d/%d, Score %.3f, Time %.3f" % \
                    (i, len(img_list_visible), target_score, spf)
            else:
                print "Frame %d/%d, Overlap %.3f, Score %.3f, Time %.3f" % \
                    (i, len(img_list_visible), overlap_ratio(gt[i],result_bb[i])[0], target_score, spf)
        iou_result[i]= overlap_ratio(gt[i],result[i])[0]
        iou_result_bb[i]= overlap_ratio(gt[i],result_bb[i])[0]


    fps = len(img_list_visible) / spf_total
    #fps = (len(img_list)-1) / spf_total #no first frame
    return iou_result, iou_result_bb, result, result_bb, fps
