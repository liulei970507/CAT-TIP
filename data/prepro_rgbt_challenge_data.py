...
from collections import OrderedDict
 ...
 # set datasets GTOT or RGBT234.
for set_type in ['GTOT','RGBT234']:
    seq_home = '/DATA/'+set_type +'/'
    for challenge_type in ['TC', 'FM', 'OCC', 'ILL', 'SC', 'ALL']: #['TC', 'FM', 'OCC', 'ILL', 'SC']: # set challenge type. FM~Fast	Motion; OCC~Occlusion; SC~Scale; Variation; ILL~Illumination Variation; TC~Thermal Crossover; ALL~Whole dataset
        
        if set_type=='GTOT':
            seqlist_path = '/DATA/GTOT.txt'
            output_path = './GTOT_'+challenge_type+'.pkl'
        elif set_type == 'RGBT234':
            seqlist_path = '/DATA/RGBT234.txt'
            output_path = './RGBT234_'+challenge_type+'.pkl'
        ...
        for i, seqname in enumerate(seq_list):
            ...
            if challenge_type == 'FM':
                ...
                challenge_inf = 'illum_change.tag'#'illum_change_random.tag'
            elif challenge_type == 'TC':
                challenge_inf = 'thermal_crossover.tag'#'thermal_crossover_random.tag'
            ...
            if challenge_type!='ALL':
                try:
                    ...
                    #print(seqname,challenge_type,len(img_list_v),len(gt))
                    if len(img_list_v)>0:
                        print(img_list_v)
                        data[seqname] = {'images_v': img_list_v, 'images_i': img_list_i,'gt': gt}
                    else:
                        print(seqname,'length not enough!')
                except:
                    print(seqname,'no',challenge_type)
        # Save db
