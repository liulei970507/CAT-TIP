...
if __name__ == '__main__':
    for dataset in ['GTOT', 'RGBT210', 'RGBT234', 'LasHeR']:
        for method in ['gamma1']:
            data_path = '/data/' + dataset
            data_dir = glob(os.path.join(data_path,'*'))
            data_dir.sort()
            save_path = '/data/liulei/pytracking_cat/data_generation/' + dataset + '_MB_' + method +'/'
                ...
                try:
                    ...
                    for count, t_frame in enumerate(t_seq_dir):
                        ...
                        if flag[0] == 1 and count > 0:
                            ...
                            res_t = motion_blur1(t_array, length, angle)
                            res_rgb = motion_blur1(rgb_array, length, angle)
                        else:
                            res_t = t_array
                            res_rgb = rgb_array
