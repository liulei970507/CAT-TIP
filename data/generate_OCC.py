...
if __name__ == '__main__':    
    for dataset in ['LasHeR']:
        for method in ['gamma1']:
            ...
            data_dir.sort()
            save_path = '/DATA/liulei/pytracking/data_generation/' + dataset + '_OCC_' + method +'/'
            if not os.path.exists(save_path):
                os.mkdir(save_path)
            for i in data_dir:
                try:
                    print(dataset, method, i)
                    if i!='/DATA/LasHeR/ab_rightlowerredcup_quezhen':
                        continue
                    print('ab_rightlowerredcup_quezhen gogogo!!!')
                    ...
                    for count, t_frame in enumerate(t_seq_dir):
                        ...
                        frame_num_idx = count
                        while True:
                            if gt[frame_num_idx].tolist()[2] == 0. or gt[frame_num_idx].tolist()[3] == 0.:
